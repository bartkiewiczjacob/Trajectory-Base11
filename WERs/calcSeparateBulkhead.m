function [wTot] = calcSeparateBulkhead(id, p, mCH4, mLOX)

%% Description

% This function calculates the weight of separate bulkhead tanks with given
...
hCH4 = (vCH4 - (4 / 3 * pi * (id / 2) .^ 3)) ./ (pi * (id / 2) .^ 2);
hLOX = (vLOX - (4 / 3 * pi * (id / 2) .^ 3)) ./ (pi * (id / 2) .^ 2);

% weights of parts of tanks
domeCH4 = 4 / 3 * pi * rhoAl * ((odCH4 / 2) .^ 3 - (id / 2) .^ 3);
