...
for prop = 1
    ...
    for Thrust = linspace(3000, 6000, n);
        i = 1;
        for Pc = linspace(500, 850, n)
            ...
            for OF = linspace(2, 3, n) 
                ...
                mass_propellant_0 = 200000/ISP; %Calculates propellent mass based on 
                burn_time = 200000/ Thrust; %[sec]
                ...
                tank = tank_mass(propellant, Pc + 250, diameter, material(1), max_q, max_hydro_p);
                D.Pc = Pc;
                D.ox = 'oxygen';
                ...
                mass_payload = 5;
                wet_mass = mass_propellant + mass_inert + mass_payload;%[lbs]
                lambda = mass_propellant/wet_mass;
                eul_0 = [0,5*pi/180,0]; %initial orientation [rad]
                k_quat = 1; 
                quat_statename = 'quat';
                initial_height = 0; %[ft]
                data = load('aerodata.csv');
                ...
                I_dry = 1/12 * dry_mass_slugs * [3*(diameter_ft/2)^2, 0, 0; ...
                                       ...
                                       0, 0, rocket_length^2 + 3*(diameter_ft/2)^2]; %[dry mass Inertia Tensor]
                sim('Purdue_Sim'); % runs the simulation
                Pe = pressure_integral(end)/burn_time;
                max_q = max(q)/144;
                height_max_OF(prop, j, i, k) = max(altitude);
                P_c(prop, j, i, k) = Pc;
